...
def watchDog_Thread():
	...
	while True:
		...
		print("[%s]unconnectedTime : %s " % (cur_time(),str(unconnectedTime)))
		print("[%s] connected : %s" % (cur_time(),str(connected)))
		print("[%s] pid : %s" % (cur_time(),str(pid)))
		now = int(time.time())
		if (unconnectedTime is not None) and ((now - unconnectedTime) > 60): #could also check connected is False, but this is more fault proof
			...
			print("[%s] PSTree: %s" % (cur_time(),pstree))
			try:
				...
				print("[%s] Couldn't find pid of bluepy-helper" % (cur_time(),))
			os.system("kill " + bluepypid)
			print("[%s] Killed bluepy with pid: %s" % (cur_time(),str(bluepypid)))
	...
	while True:
		try:
			...
			if (mea == previousMeasurement and identicalCounter < args.skipidentical): #only send data when it has changed or X identical data has been skipped, ~10 pakets per minute, 50 pakets --> writing at least every 5 minutes
				print("[%s] Measurements are identical don't send data\n" % (cur_time(),))
				identicalCounter+=1
				continue
			identicalCounter=0
			...
			fmt +=",timestamp"
			if args.callback.split(".")[1] == "py":
				cmd = "python3 " + path + "/" + args.callback + " " + fmt + " " + params
			else:
			...
			if (ret != 0):
					...
					print("[%s] Data couln't be send to Callback, retrying..." % (cur_time(),))
...
def keepingLEScanRunning(): #LE-Scanning gets disabled sometimes, especially if you have a lot of BLE connections, this thread periodically enables BLE scanning again
	...
	while True:
		...
		if now - lastBLEPaketReceived > args.watchdogtimer:
			print("[%s] Watchdog: Did not receive any BLE Paket within %s s. Restarting BLE scan. Count: %s" % (cur_time(),int(now - lastBLEPaketReceived),BLERestartCounter))
			disable_le_scan(sock)
...
class MyDelegate(btle.DefaultDelegate):
	...
	def handleNotification(self, cHandle, data):
		...
		try:
			...
			#print("[%s] Temp received: " + str(temp))
			if args.round:
				#print("[%s] Temperatur unrounded: " + str(temp
				...
				if args.debounce:
					...
					if mode=="trunc": #only a few times
						...
						temp=round(temp,0)
					temp /=10.
					#print("[%s] Debounced temp: " + str(temp))
				else:
					temp=round(temp,1)
			humidity=int.from_bytes(data[2:3],byteorder='little')
			print("[%s] Temperature: %s" % (cur_time(),str(temp)))
			print("[%s] Humidity: %s" % (cur_time(),str(humidity)))
			voltage=int.from_bytes(data[3:5],byteorder='little') / 1000.
			print("[%s] Battery voltage: %sV" % (cur_time(),voltage))
			...
			if args.battery:
				...
				print("[%s] Battery level: %s"  % (cur_time(),batteryLevel))
				

			if args.offset:
				humidityCalibrated = humidity + args.offset
				print("[%s] Calibrated humidity: %s" % (cur_time(),str(humidityCalibrated)))
...
def connect():
	#print("[%s] Interface: " + str(args.interface))
...
if args.device:
	if re.match("[0-9a-fA-F]{2}([:]?)[0-9a-fA-F]{2}(\\1[0-9a-fA-F]{2}){4}$",args.device):
		...
		print("[%s] Please specify device MAC-Address in format AA:BB:CC:DD:EE:FF" %s (cur_time(),))
		os._exit(1)
elif not args.atc:
	parser.print_help()
	os._exit(1)
...
if args.TwoPointCalibration:
	if(not(args.calpoint1 and args.offset1 and args.calpoint2 and args.offset2)):
		print("[%s] In 2 Point calibration you have to enter 4 points" % (cur_time(),))
		os._exit(1)
	elif(args.offset):
		print("[%s] Offset calibration and 2 Point calibration can't be used together" % (cur_time(),))
		os._exit(1)
if not args.name:
...
if args.device: 
	...
	print("[%s] watchdogThread started" % (cur_time(),))
		...
		try:
			if not connected:
				...
				# if bluepypid is not None:
					# os.system("kill " + bluepypid)
					# print("[%s] Killed possibly remaining bluepy-helper")
				# else:
					# print("[%s] bluepy-helper couldn't be determined, killing not allowed")
						
				print("[%s] Trying to connect to %s"  % (cur_time(),adress))
				p=connect()
				# print("[%s] Own PID: "  + str(pid))
				# pstree=os.popen("pstree -p " + str(pid)).read() #we want to kill only bluepy from our own process tree, because other python scripts have there own bluepy-helper process
				# print("[%s] PSTree: " + pstree)
				# try:
					# bluepypid=re.findall(r'bluepy-helper\((.*)\)',pstree)[0] #Store the bluepypid, to kill it later
				# except IndexError: #Should not happen since we're now connected
					# print("[%s] Couldn't find pid of bluepy-helper")				
				connected=True
				unconnectedTime=None
				
			# if args.battery:
					# if(cnt % args.battery == 0):
						# print("[%s] Warning the battery option is deprecated, Aqara device always reports 99 % battery")
						# batt=p.readCharacteristic(0x001b)
						# batt=int.from_bytes(batt,byteorder="little")
						# print("[%s] Battery-Level: " + str(batt))
			...
			print("[%s] Connection lost" % (cur_time(),))
			...
			if args.unreachable_count != 0 and connectionLostCounter >= args.unreachable_count:
				print("[%s] Maximum numbers of unsuccessful connections reaches, exiting" % (cur_time(),))
				os._exit(0)
			time.sleep(1)
			print(e)
			print(traceback.format_exc())		
		...
		def le_advertise_packet_handler(mac, adv_type, data, rssi):
			...
			if(atcIdentifier == "1A18" and ATCPaketMAC == macStr) and not args.onlydevicelist or (atcIdentifier == "1A18" and mac in sensors): #only Data from ATC devices, double checked
				...
				if lastAdvNumber == None or lastAdvNumber != advNumber:
					...
					#print("[%s] BLE packet: %s %02x %s %d" % (mac, adv_type, data_str, rssi))
					#print("[%s] AdvNumber: ", advNumber)
					...
					#print("[%s] Temperature: ", temperature)
